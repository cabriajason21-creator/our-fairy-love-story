...
export default function Finale({ finaleMessage, theme }: FinaleProps) {
  ...
  const triggerConfetti = () => {
    // Light pink, blue, orange, and gold colors matching our palette
    ...
    // Clean up particles after they fall out of view
    setTimeout(() => {
      setConfetti([]);
    }, 5500);
  };
  ...
  return (
    ...
    >
      <div className="w-full max-w-[780px] bg-[#FAF4E9] border-[4px] border-[#4E2512] rounded-[32px] p-6 sm:p-10 text-center shadow-[0_8px_0_#4E2512] relative my-auto max-h-[82vh] flex flex-col justify-between overflow-hidden">
        ...
        <div className="shrink-0 mb-4">
          ...
          <div className="h-[3px] w-28 bg-[#4E2512] mx-auto mb-4 rounded-full" />
        </div>

        {/* Message Container - RPG scroll parchment style */}
        <div 
          className={`flex-1 bg-[#FCF6E9] border-2 border-[#4E2512] rounded-2xl p-6 sm:p-8 shadow-[inset_0_2px_5px_rgba(0,0,0,0.06)] overflow-y-auto max-h-[48vh] min-h-[280px] sm:min-h-[380px] text-left relative ${
            unlocked ? "" : "flex flex-col items-center justify-center text-center"
          }`}
        >
          <p
            className={`font-serif text-sm sm:text-base md:text-[17px] font-bold leading-relaxed sm:leading-loose whitespace-pre-wrap transition-all duration-700 select-text ${
              unlocked 
                ? `opacity-100 scale-100 pb-8 ${theme === "night" ? "text-[#FAF4E9]" : "text-[#4E2512]"}` 
                : "opacity-60 scale-95 font-black text-base sm:text-lg text-[#EA580C]/80"
            }`}
          >
        ...
        {!unlocked && (
          <div className="shrink-0 mt-5 flex justify-center">
            <button
              ...
              className="w-full max-w-xs bg-[#EA580C] hover:bg-[#C2410C] text-white font-serif font-black text-sm px-8 py-3.5 rounded-2xl border-3 border-[#4E2512] shadow-[0_4px_0_#4E2512] active:translate-y-0.5 active:shadow-[0_1px_0_#4E2512] transition-all cursor-pointer uppercase tracking-wider"
            >
              🔓 Unlock Message 🔓
            </button>
          </div>
